...
export async function getExtractor() {
  if (!extractorInstance) {
    try {
      console.log(`[EXPRESS EMBEDDING] Loading ONNX model: ${MODEL_NAME}...`);
      extractorInstance = await pipeline('feature-extraction', MODEL_NAME);
      console.log('[EXPRESS EMBEDDING] ONNX Model ready.');
    } catch (err) {
      console.warn('[EXPRESS EMBEDDING WARNING] Falling back to deterministic embedding:', err.message);
